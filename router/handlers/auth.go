...
func (s Server) LoginHandler(c *macaron.Context) {
	...
	if err == auth.ErrInvalidCredentials {
		...
		c.Data["response"] = apiModels.Error{Code: http.StatusUnauthorized, Message: "Wrong credentials or account does not exist"}
		return
	}
	if err != nil {
...
func (s Server) SignupHandler(c *macaron.Context) {
	...
	if err == auth.ErrInvalidUserData || err == auth.ErrUserAlreadyExists {
		c.Data["response"] = apiModels.Error{Code: http.StatusBadRequest, Message: err.Error()}
		return
	} else if err != nil {
		c.Data["response"] = err
		return
