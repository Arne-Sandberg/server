...
func main() {
	...
	database, err := db.NewStormDB(config.GetString("db.type"), config.GetString("db.host"), config.GetInt("db.port"), config.GetString("db.user"), config.GetString("db.password"), config.GetString("db.name"))
	if err != nil {
