...
func TestUserRepository(t *testing.T) {
	...
	// Check for correct counts of inserted users
	adminCount, err = rep.AdminCount()
	if err != nil {
		t.Errorf("Failed to get admin count: %v", err)
	}
	...
	// Check for correctly reading back
	...
	//  Check for errors when reading back deleted user
	readBackUser, err = rep.GetByID(user1.ID)
