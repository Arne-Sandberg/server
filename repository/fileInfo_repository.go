...
func (rep *FileInfoRepository) GetByPath(userID int64, path, name string) (fileInfo *models.FileInfo, err error) {
	...
	err = databaseConnection.Raw(getByPath, path, name, userID, userID).Scan(fileInfo).Error
	if err != nil {
...
func (rep *FileInfoRepository) Search(userID int64, path, name string) (results []*models.FileInfo, err error) {
	pathSearch := path + "%"
	fileNameSearch := "%" + name + "%"
	err = databaseConnection.Raw(getSearch, pathSearch, fileNameSearch, userID, userID).Order(fileListOrder).Find(&results).Error
	if err != nil && IsRecordNotFoundError(err) {
		err = nil
	} else if err != nil {
